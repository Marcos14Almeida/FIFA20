...
public class ActivityResultadosChampions extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_resultados_champions);
        ...
        if(rodada>=16 && rodada<=21){//Champions League
            for (int i=1;i<=32;i++){
                ...
                for(int z=1; z<=4;z=z+2){//z+2 pega cada confronto chave1, chave3
                    if(chave.get(z)==posicaoSIMULACAO ) {timea=z;timeb=z+1;SIMULAR=1;    //timea entre 1 e 4
                        if(SIMULAR==1){
                            if((chave.get(z)==minhaposicaoChampions || chave.get(z)==minhaposicaoChampions) &&  grupoSIMULACAO == meuGrupoChampions){SIMULAR=0;soma++;
                                break;}
                            ...
                            tabela((4 * (grupoSIMULACAO - 1))+timea,1);
                            tabela((4 * (grupoSIMULACAO - 1))+timeb,2);
                        }}}
            }
        }
        globalVariable.DefinirOrdemChampions();

        //SIMULAÇÃO LIBERTADORES
        if(rodada>=16 && rodada<=21){//Champions League
            for (int i=1;i<=32;i++){
                grupoSIMULACAO = 1+(i/4); //-> i=7 res:0+1  *i=8 res:2
                int posicaoSIMULACAO= (i%(4));     //-> i=7 res:7   *i=8 res=0
    ...
    public void PrintarTime(Integer id,Integer GMouGS){
        final GlobalClass globalVariable = (GlobalClass) getApplicationContext();
        if(GMouGS==1){
            globalVariable.imageLogo(timesChampions.get((4 * (grupoSIMULACAO - 1))+chave.get(timea)),imagemTime);
            textoNomeTime.setText(timesChampions.get((4 * (grupoSIMULACAO - 1))+chave.get(timea)));
            textoGolsMarcados.setText(String.valueOf(golsMarcados));}
        if(GMouGS==2){
            globalVariable.imageLogo(timesChampions.get((4 * (grupoSIMULACAO - 1))+chave.get(timeb)),imagemTime);
            textoNomeTime.setText(timesChampions.get((4 * (grupoSIMULACAO - 1))+chave.get(timeb)));
            textoGolsMarcados.setText(String.valueOf(golsSofridos));}
    }

    public void tabela(Integer id,Integer GMouGS){
         /*////////////////////////////////////////////////////////////////////////////////////////////////////////////
                // TABELA //
         *//////////////////////////////////////////////////////////////////////////////////////////////////////////////
        //GRUPO A
        if(id==1){
        textoNomeTime =  findViewById(R.id.nomeTime1A);
        textoGolsMarcados = findViewById(R.id.textoGols1A);
        imagemTime = findViewById(R.id.logo1A);
        }
        if(id==2){
            textoNomeTime =  findViewById(R.id.nomeTime2A);
            textoGolsMarcados = findViewById(R.id.textoGols2A);
            imagemTime = findViewById(R.id.logo2A);
           }
        if(id==3){
        textoNomeTime =  findViewById(R.id.nomeTime3A);
        textoGolsMarcados = findViewById(R.id.textoGols3A);
        imagemTime = findViewById(R.id.logo3A);
       }
        if(id==4){
        textoNomeTime =  findViewById(R.id.nomeTime4A);
        textoGolsMarcados = findViewById(R.id.textoGols4A);
        imagemTime = findViewById(R.id.logo4A);
       }
        //GRUPO B
        if(id==5){
